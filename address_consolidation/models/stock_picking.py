...
class StockPicking(models.Model):
    ...
    def _get_invoice_vals(
            self, key, inv_type, journal_id, move, picking=None):
        """Override method to pass historical address data to invoice.
        Called by stock_picking._invoice_create_line()
        No correct way to get the picking record,
        so override _invoice_create_line() below.
        """
        ...
        res.update({
            ...
            'invoice_partner_country_id':
                picking.shipping_partner_country_id.id,
        })
        return res

    @api.model
    def _invoice_create_line(
            self, moves, journal_id, inv_type='out_invoice', context=None):
